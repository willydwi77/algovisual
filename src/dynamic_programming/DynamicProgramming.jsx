...
import { Play, Pause, RotateCcw, BrainCircuit, Code, Variable, MessageSquare, SkipBack, SkipForward, StepBack, StepForward, Square, Table, Award, Layers, Clock, Info, Percent } from 'lucide-react'
...
const DynamicProgramming = () => {
  ...
  const variableDefinitions = {
    ...
    knapsack: { W: 'Kapasitas', w: 'Bobot Saat Ini', i: 'Item Ke-', val: 'Nilai', weight: 'Bobot' },
    lcs: { m: 'Panjang Str1', n: 'Panjang Str2', i: 'Indeks Str1', j: 'Indeks Str2', charX: 'Karakter 1', charY: 'Karakter 2' },
    coin: { amount: 'Target Nilai', coins: 'Daftar Koin', i: 'Nilai Target', coin: 'Koin Aktif' },
    matrix: { dims: 'Dimensi', i: 'Awal Chain', j: 'Akhir Chain', k: 'Titik Potong', cost: 'Biaya' }
  }
  ...
  const algorithmDescriptions = {
    fibonacci: {
      ...
      description: 'Menghitung bilangan Fibonacci menggunakan metode (bottom-up). Teknik ini menghindari rekursi berulang yang tidak efisien dengan menyimpan hasil perhitungan sebelumnya dalam tabel memori.',
      complexity: 'Time: O(n), Space: O(n)',
      useCase: 'Pengenalan DP, optimasi urutan bilangan, masalah pertumbuhan populasi.',
...
Return dp[n]`
    ...
    knapsack: {
      ...
      description: 'Menentukan kombinasi barang dengan nilai tertinggi yang dapat dimuat dalam kapasitas tertentu. Menggunakan tabel 2D untuk menyimpan nilai maksimum pada setiap kapasitas dan subset barang.',
      complexity: 'Time: O(n × W), Space: O(n × W)',
      useCase: 'Optimasi anggaran, alokasi sumber daya, pemilihan portofolio investasi.',
  ...
  For w from 1 to W:
    ...
    lcs: {
      ...
      description: 'Mencari urutan karakter terpanjang yang muncul pada kedua string dalam urutan yang sama (tidak harus bersebelahan). Sangat berguna dalam perbandingan teks dan bioinformatika.',
  ...
  For j from 1 to n:
    ...
    coin: {
      ...
      description: 'Menghitung berapa banyak cara untuk menghasilkan nilai uang tertentu dari denominasi koin yang tersedia. Contoh klasik tipe unbounded knapsack.',
...
For each coin in coins:
  For i from coin to amount:
    ...
    matrix: {
      ...
      description: 'Menentukan urutan perkalian matriks yang paling efisien (meminimalkan operasi skalar).',
      complexity: 'Time: O(n³), Space: O(n²)',
  ...
  const snapshot = (table, activeCell, dependencies, formula, desc, vars, solution = null, activeLine = -1) => ({
  ...
  const generateSteps = (algoType) => {
    ...
    if (algoType === 'fibonacci') {
      ...
      stepsArr.push(snapshot([dp], null, [], '', 'Inisialisasi: Base case dp[0]=0, dp[1]=1', { n }, null, 2))
      ...
      for (let i = 2; i <= n; i++) {
        ...
        stepsArr.push(snapshot([dp], [0, i], [], `dp[${i}] = ${dp[i]}`, `dp[${i}] = ${dp[i - 1]} + ${dp[i - 2]} = ${dp[i]}. Nilai tersimpan.`, { i, result: dp[i] }, null, 6))
      }

      stepsArr.push(snapshot([dp], null, [], '', `✓ Selesai! Fibonacci(${n}) = ${dp[n]}.`, { n, result: dp[n] }, dp[n], 9))
      ...
      stepsArr.push(snapshot(dp, null, [], '', 'Inisialisasi tabel DP. dp[i][w] = nilai maksimal dengan i item & kapasitas w', { n, W }, null, 2))
      ...
      for (let i = 1; i <= n; i++) {
        for (let w = 1; w <= W; w++) {
          ...
          if (item.weight <= w) {
            ...
            stepsArr.push(snapshot(dp, [i, w], [], `dp[${i}][${w}] = ${dp[i][w]}`, `Pilih yang lebih baik: ${dp[i][w]}.`, { result: dp[i][w] }, null, 7))
          } else {
            stepsArr.push(snapshot(dp, [i, w], [[i - 1, w]], `dp[${i}][${w}] = dp[${i - 1}][${w}]`, `Item ${i} terlalu berat. Skip.`, { i, w, weight: item.weight }, null, 12))
            dp[i][w] = dp[i - 1][w]
          }
        }
      }

      stepsArr.push(snapshot(dp, [n, W], [], '', `✓ Selesai! Nilai maksimal = ${dp[n][W]}.`, { maxValue: dp[n][W] }, dp[n][W], 16))
      ...
      stepsArr.push(snapshot(dp, null, [], '', `Mencari LCS dari "${X}" dan "${Y}".`, { m, n }, null, 2))

      for (let i = 1; i <= m; i++) {
        for (let j = 1; j <= n; j++) {
          if (X[i - 1] === Y[j - 1]) {
            stepsArr.push(snapshot(dp, [i, j], [[i - 1, j - 1]], `dp[${i}][${j}] = dp[${i - 1}][${j - 1}] + 1`, `Karakter match! X[${i - 1}]='${X[i - 1]}' = Y[${j - 1}]='${Y[j - 1]}'.`, { i, j, charX: X[i - 1], charY: Y[j - 1] }, null, 7))
            dp[i][j] = dp[i - 1][j - 1] + 1
          } else {
            stepsArr.push(
              snapshot(
          ...
          stepsArr.push(snapshot(dp, [i, j], [], `dp[${i}][${j}] = ${dp[i][j]}`, `LCS saat ini ${dp[i][j]}.`, { result: dp[i][j] }, null, 7))
        }
      }

      stepsArr.push(snapshot(dp, [m, n], [], '', `✓ Selesai! Panjang LCS = ${dp[m][n]}.`, { lcsLength: dp[m][n] }, dp[m][n], 16))
      ...
      stepsArr.push(snapshot([dp], null, [], '', `Hitung cara membuat amount ${amount} dengan koin ${coins.join(', ')}.`, { amount, coins: coins.join(',') }, null, 2))
      stepsArr.push(snapshot([dp], [0, 0], [], 'dp[0] = 1', 'Base case: 1 cara untuk 0 (tidak ambil koin).', { 'dp[0]': 1 }, null, 3))

      for (let coinIdx = 0; coinIdx < coins.length; coinIdx++) {
        const coin = coins[coinIdx]
        stepsArr.push(snapshot([dp], null, [], '', `Proses koin ${coin}.`, { coin, coinIdx: coinIdx + 1 }, null, 5))

        for (let i = coin; i <= amount; i++) {
          const oldVal = dp[i]
          stepsArr.push(snapshot([dp], [0, i], [[0, i - coin]], `dp[${i}] += dp[${i - coin}]`, `Untuk nilai ${i}, tambahkan cara dari dp[${i - coin}].`, { i, coin, 'dp[i-coin]': dp[i - coin], oldValue: oldVal }, null, 7))

          dp[i] += dp[i - coin]

          stepsArr.push(snapshot([dp], [0, i], [], `dp[${i}] = ${dp[i]}`, `Total cara untuk ${i} sekarang ${dp[i]}.`, { result: dp[i] }, null, 7))
        }
      }

      stepsArr.push(snapshot([dp], [0, amount], [], '', `✓ Selesai! Ada ${dp[amount]} cara.`, { totalWays: dp[amount] }, dp[amount], 11))
      ...
      stepsArr.push(snapshot(dp, null, [], '', `Matrix dimensions: ${dims.join(' × ')}. Cari urutan perkalian optimal.`, { n, dims: dims.join('×') }, null, 3))

      for (let len = 2; len <= n; len++) {
        stepsArr.push(snapshot(dp, null, [], '', `Chain length ${len}.`, { len }, null, 6))

        for (let i = 0; i < n - len + 1; i++) {
          const j = i + len - 1
          dp[i][j] = Infinity

          stepsArr.push(snapshot(dp, [i, j], [], `dp[${i}][${j}]`, `Cari split optimal untuk M${i}..M${j}.`, { i, j, startDim: dims[i], endDim: dims[j + 1] }, null, 9))
          ...
          for (let k = i; k < j; k++) {
            ...
            if (cost < dp[i][j]) {
              dp[i][j] = cost
              stepsArr.push(snapshot(dp, [i, j], [], `dp[${i}][${j}] = ${cost}`, `Update! Split di k=${k} lebih baik.`, { bestK: k, result: cost }, null, 13))
            }
          }

          stepsArr.push(snapshot(dp, [i, j], [], `dp[${i}][${j}] = ${dp[i][j]}`, `Optimal untuk M${i}..M${j} = ${dp[i][j]}.`, { result: dp[i][j] }, null, 13))
        }
      }

      stepsArr.push(snapshot(dp, [0, n - 1], [], '', `✓ Selesai! Minimum operasi = ${dp[0][n - 1]}.`, { minOps: dp[0][n - 1] }, dp[0][n - 1], 17))
    }
        ...
        {lines.map((line, index) => {
          ...
          return (
            <div
              ...
              <span className={`whitespace-pre ${isActive ? 'text-indigo-200 font-bold' : 'text-slate-300'}`}>{line}</span>
            </div>
          )
        })}
      </div>
  ...
  const ExecutionLog = () => {
    ...
    return (
      <div className='flex flex-col h-full bg-slate-900 overflow-hidden'>
        ...
        <div ref={logRef} className='flex-1 overflow-auto p-4 space-y-2 scrollbar-thin scrollbar-thumb-slate-700'>
          {logs.map((step, idx) => (
            <div key={idx} className={`text-xs p-2 rounded border-l-2 ${idx === logs.length - 1 ? 'bg-orange-900/30 border-orange-500 ring-1 ring-orange-500/20' : 'bg-slate-800/50 border-slate-600'}`}>
        ...
        <header className='w-full max-w-7xl mb-6 flex flex-col gap-4 border-b border-slate-700 pb-4'>
          <div className='flex flex-col md:flex-row justify-between items-center gap-4'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                <h1 className='text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-orange-400 to-amber-300 pb-2 mb-1'>Algo Dynamic</h1>
                <p className='text-xs text-slate-400'>Optimasi dengan Tabel & Memo</p>
              </div>
            </div>
            ...
            <div className='flex flex-wrap gap-3 items-center bg-slate-800 p-2 rounded-xl border border-slate-700 justify-center'>
              ...
              {algorithm === 'fibonacci' && (
                <div className='flex items-center gap-2 px-2'>
                  ...
                  />
                  <span className='text-sm font-mono text-orange-400 w-6 text-center'>{fibN}</span>
                </div>
              )}
              {algorithm === 'knapsack' && (
                 <div className='flex items-center gap-2 px-2'>
                  ...
                  <input
                    ...
                    className='w-20 h-2 bg-slate-700 rounded-lg accent-orange-500 cursor-pointer'
                  />
                  <span className='text-sm font-mono text-orange-400 w-6 text-center'>{knapsackCapacity}</span>
                </div>
              )}
              {algorithm === 'coin' && (
                <div className='flex items-center gap-2 px-2'>
                  ...
                  <input
                    ...
                    className='w-20 h-2 bg-slate-700 rounded-lg accent-orange-500 cursor-pointer'
                  />
                  <span className='text-sm font-mono text-orange-400 w-6 text-center'>{coinAmount}</span>
                </div>
              )}

              {/* Time Display */}
              <div className='px-3 py-1 bg-slate-900 rounded border border-slate-600 flex flex-col items-center min-w-[80px]'>
                <span className='text-[10px] text-slate-500 uppercase font-bold flex items-center gap-1'><Clock size={10}/> Waktu</span>
            ...
            <div className='bg-slate-900 border border-slate-700 rounded-xl overflow-hidden shadow-xl p-4 flex flex-col gap-4'>
               ...
               {currentVisual.currentFormula && (
                 <div className='bg-slate-950/50 rounded-lg p-3 border border-slate-800'>
                    ...
                    <div className='font-mono text-orange-300 text-center text-sm'>{currentVisual.currentFormula}</div>
                 </div>
               )}
              ...
              <div className='bg-slate-800/80 p-2 rounded-xl border border-slate-600 flex justify-center items-center gap-4 shadow-lg'>
                <div className='flex items-center gap-1'>
                  <button onClick={handleStop} className='p-2 hover:bg-red-500/20 text-slate-300 hover:text-red-400 rounded-lg' title='Stop'>
                    <Square size={20} fill='currentColor' />
                  </button>
                  <div className='w-px h-6 bg-slate-600 mx-2'></div>
                  <button onClick={handleBegin} className='p-2 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg' title='Start'>
                    <SkipBack size={20} />
                  </button>
                  <button onClick={handlePrev} disabled={currentStep === 0} className='p-2 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg disabled:opacity-30' title='Prev'>
                    <StepBack size={20} />
                  </button>
                   <button onClick={() => setIsPlaying(!isPlaying)} className={`p-3 rounded-full shadow-lg ${isPlaying ? 'bg-orange-500' : 'bg-orange-600'} text-white`}>
                    {isPlaying ? <Pause size={24} fill='currentColor' /> : <Play size={24} fill='currentColor' className='ml-1' />}
                  </button>
                   <button onClick={handleNext} disabled={currentStep === steps.length - 1} className='p-2 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg disabled:opacity-30' title='Next'>
                    <StepForward size={20} />
                  </button>
                   <button onClick={handleEnd} className='p-2 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg' title='End'>
                    <SkipForward size={20} />
                  </button>
                </div>
                 <div className='hidden md:flex flex-col flex-1 max-w-xs ml-4'>
                  <div className='flex justify-between text-[10px] text-slate-400 mb-1'>
                    <span>Progress</span>
                    <span>{currentStep} / {steps.length - 1}</span>
                  </div>
                  <div className='w-full bg-slate-700 h-1.5 rounded-full overflow-hidden'>
                     <div className='bg-orange-500 h-full transition-all duration-100' style={{ width: `${(currentStep / (steps.length - 1 || 1)) * 100}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className='bg-slate-900 border border-slate-700 rounded-xl overflow-hidden shadow-xl'>
                ...
                <div className='p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 bg-[#151925]'>
                  {Object.entries(currentVisual.variables).map(([key, val]) => (
                     <VarBadge key={key} name={key} value={typeof val === 'object' ? JSON.stringify(val) : val} />
                  ))}
                </div>
              </div>
            )}
