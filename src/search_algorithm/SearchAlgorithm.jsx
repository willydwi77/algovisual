...
const ALGO_INFO = {
    linear: {
        title: "LINEAR SEARCH",
        description:
            "Algoritma pencarian paling sederhana yang memeriksa setiap elemen dalam array secara berurutan dari awal hingga akhir sampai menemukan elemen yang dicari atau mencapai akhir array.",
        complexity: "O(n)",
    ...
    binary: {
        title: "BINARY SEARCH",
        description:
            "Algoritma pencarian efisien yang bekerja pada array terurut dengan membagi area pencarian menjadi dua bagian pada setiap iterasi, menghilangkan setengah dari elemen yang tersisa.",
        complexity: "O(log n)",
    ...
    jump: {
        title: "JUMP SEARCH",
        description:
            "Algoritma pencarian yang melompati sejumlah elemen tetap (√n) alih-alih mencari setiap elemen. Setelah menemukan blok yang tepat, dilakukan pencarian linear di dalam blok tersebut.",
        complexity: "O(√n)",
    ...
    interpolation: {
        ...
        description:
            "Peningkatan dari binary search untuk data yang didistribusikan secara seragam. Menghitung posisi yang mungkin dari elemen yang dicari menggunakan interpolasi.",
    ...
    exponential: {
        ...
        description:
            "Algoritma pencarian yang menemukan rentang di mana elemen mungkin ada dengan menggandakan indeks secara eksponensial, kemudian melakukan binary search pada rentang tersebut.",
        complexity: "O(log n)",
...
const CodeViewer = ({ code, activeLine }) => {
    ...
    const highlightCodePart = (text) => {
        ...
        return words.map((word, idx) => {
            ...
            const keywords = ['void', 'int', 'bool', 'vector', 'for', 'while', 'if', 'else', 'return', 'swap', 'break', 'continue', 'min'];
    ...
    const generateSteps = (initialArray, algo, target) => {
            ...
            while (prev < n && arr[Math.min(step, n) - 1] < target) {
                s.push(
                    snapshot(
                        ...
                        checked,
                        8,
                        `Periksa arr[${Math.min(step, n) - 1}] = ${arr[Math.min(step, n) - 1]} < ${target}?`,
                        target
                    )
                );
                ...
                prev = step;
                const newStep = step + Math.floor(Math.sqrt(n));
                ...
                s.push(
                    snapshot(
                        ...
                        checked,
                        9,
                        `Lompat ke blok berikutnya: prev = ${prev}`,
                        target
                    )
                );
            ...
            while (low <= high && target >= arr[low] && target <= arr[high]) {
                if (low === high) {
                    s.push(
                        snapshot(
                            ...
                            checked,
                            7,
                            `Hanya satu elemen tersisa: arr[${low}] = ${arr[low]}`,
                            target
                        )
                    );
                ...
                s.push(
                    snapshot(
                        ...
                        checked,
                        13,
                        `Interpolasi: pos = ${pos}, arr[${pos}] = ${arr[pos]}`,
                        target
                    )
                );
            ...
            if (arr[0] === target) {
                ...
                s.push(
                    snapshot(
                        ...
                        checked,
                        14,
                        `Binary search dalam rentang [${start}, ${bound}]`,
                        target
                    )
                );
                ...
                while (left <= right) {
                    ...
                    s.push(
                        snapshot(
                            ...
                            checked,
                            3,
                            `Binary: mid = ${mid}, arr[${mid}] = ${arr[mid]}`,
                            target
                        )
                    );
        ...
        s.push(
            snapshot(
                ...
                checked,
                -1,
                finalFound >= 0 ? `Pencarian selesai: Ditemukan di indeks ${finalFound}` : "Pencarian selesai: Tidak ditemukan",
                target
            )
        );
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400 font-medium">
                            Visualisasi Algoritma Searching
                        </p>
                    </div>
                </div>
                    ...
                    <div className="bg-slate-900 border border-slate-700 rounded-xl p-3 shrink-0">
                        ...
                        <div className="flex justify-center flex-wrap gap-1">
                            {currentVisual.array.map((val, idx) => (
                                ...
                                ${currentVisual.foundIndex === idx
                                            ? "border-emerald-500 bg-emerald-900/30 text-emerald-400 shadow-lg shadow-emerald-500/30"
                        ...
                        <div className="flex gap-3 mt-3 justify-center">
                            ...
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded border bg-yellow-500/10 text-yellow-400 border-yellow-500/20">
                                CHECKING
                            </span>
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded border bg-slate-700/50 text-slate-500 border-slate-600">
                                CHECKED
                            </span>
                        </div>
                    </div>
